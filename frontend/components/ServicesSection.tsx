...
const services: Service[] = [
  {
    ...
    title: '마케팅 자동화',
    description: '6개 플랫폼 동시 발행. Facebook · Instagram · Threads · X · TikTok · YouTube까지 AI 콘텐츠 자동 생성 + 원클릭 발행.',
    features: ['6개 플랫폼 자동 발행', 'AI 카피/카드뉴스', '홍보대행 캠페인', '외부 프로젝트 연동 SDK'],
    detailedDescription: `
      6개 소셜 미디어 플랫폼에 AI 콘텐츠를 자동으로 생성하고 원클릭 발행합니다.

      지원 플랫폼: Facebook, Instagram, Threads, X(Twitter), TikTok, YouTube/Shorts.
      AI 카피라이팅 엔진이 플랫폼별 최적 길이와 톤에 맞는 콘텐츠를 자동 생성합니다.

      카드뉴스 AI 시스템으로 9슬라이드 비주얼 콘텐츠를 생성하고,
      생성 즉시 연결된 채널로 자동 발행합니다.

      홍보대행 캠페인 시스템으로 외부 프로젝트의 홍보를 의뢰받아
      AI 카피 생성 → SNS 발행 → 결과 리포트까지 자동화합니다.
      범용 SDK로 어떤 프로젝트든 5분 만에 연동 가능합니다.
    `,
    useCases: [
